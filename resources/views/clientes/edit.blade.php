<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto space-y-lg">
        ...
        <div>
            <a href="{{ route('clientes.show', $cliente) }}" class="inline-flex items-center gap-xs font-label-md text-label-md text-on-surface-variant hover:text-primary mb-md">
        ...
        <form method="POST" action="{{ route('clientes.update', $cliente) }}" class="login-card rounded-xl p-lg space-y-lg">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                <div class="md:col-span-2 space-y-xs">
                    ...
                    <input id="nombre" name="nombre" type="text" required value="{{ old('nombre', $cliente->nombre) }}" placeholder="Ej: María García"
                ...
                <div class="space-y-xs">
                    ...
                    <input id="email" name="email" type="email" required value="{{ old('email', $cliente->email) }}" placeholder="email@empresa.com"
                ...
                <div class="space-y-xs">
                    ...
                    <input id="telefono" name="telefono" type="tel" value="{{ old('telefono', $cliente->telefono) }}" placeholder="+1 (555) 000-0000"
                ...
                <div class="md:col-span-2 space-y-xs">
                    ...
                    <input id="empresa" name="empresa" type="text" value="{{ old('empresa', $cliente->empresa) }}" placeholder="Ej: Acme Corp"
                ...
                <div class="md:col-span-2 space-y-xs">
                    <label for="notas" class="block font-label-md text-label-md text-on-surface-variant">Notas <span class="font-body-sm text-body-sm text-on-surface-variant">(opcional)</span></label>
                    <textarea id="notas" name="notas" rows="4" placeholder="Información adicional, preferencias, etc."
                        class="w-full px-md py-sm rounded-lg border {{ $errors->has('notas') ? 'border-error' : 'border-outline-variant' }} bg-surface-container-lowest font-body-md text-body-md text-on-surface focus:ring-2 focus:ring-primary focus:border-primary transition-all outline-none resize-none">{{ old('notas', $cliente->notas) }}</textarea>
                </div>
            </div>

            <div class="flex items-center justify-end gap-sm pt-md border-t border-outline-variant">
                <a href="{{ route('clientes.show', $cliente) }}" class="px-lg py-sm rounded-lg border border-outline-variant font-label-md text-label-md text-on-surface-variant hover:bg-surface-container transition-colors">Cancelar</a>
