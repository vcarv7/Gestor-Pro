<div class="bg-error-container/20 rounded-xl p-lg border border-error">
    ...
    <p class="font-body-md text-body-md text-on-surface mb-md w-full">
    ...
    <form method="POST" action="{{ route('settings.destroy-account') }}" data-loading
        ...
        class="space-y-md w-full">
        @csrf
        @method('DELETE')

        <div class="space-y-xs">
            <label for="password" class="block font-label-md text-label-md text-on-surface-variant">Confirma tu contraseña para eliminar la cuenta</label>
            <input id="password" name="password" type="password"
                class="block w-full px-md py-sm rounded-lg border border-outline-variant bg-surface-container-lowest font-body-md text-body-md text-on-surface placeholder:text-outline focus:ring-2 focus:ring-error focus:border-error transition-all outline-none" />
        </div>
